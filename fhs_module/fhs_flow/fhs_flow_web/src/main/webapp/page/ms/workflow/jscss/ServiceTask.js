...
draw2d.ServiceTask.prototype.figureDoubleClick=function(){
	...
	nodeid = event.getId();
	openProperties(nodeid,'serviceTaskProperties');
};
...
//		}
//	}
	var tmp_expression = trim(this.expression);
//	alert('serviceType:' + this.serviceType);
	if(this.serviceType=="javaClass"){
		xml+=' activiti:class="'+tmp_expression+'" ';
	}
	else if(this.serviceType=="expression"){
		xml+=' activiti:expression="'+tmp_expression+'" ';
	}
	else if(this.serviceType=="delegateExpression"){
		xml+=' activiti:delegateExpression="'+tmp_expression+'" ';
	}
	if(this.resultVariable != null && this.resultVariable != ""){
		xml+=' activiti:resultVariableName="'+this.resultVariable+'"';
	}
