draw2d.CallActivity=function(_url){
	_url="plug-in/designer/icons/callactivity.png";
...
draw2d.CallActivity.prototype.toXML=function(){
	...
	if(this.insource!=null&&this.intarget!=null&&this.insource!=''&&this.intarget!=''){
		...
		//update-begin---author:scott------------date:20160308-----------------for:默认传递变量给子流程-------
		//子流程默认传递固定变量
		if(this.insource != 'applyUserId'){
			xml = xml+'<activiti:in source="applyUserId" target="applyUserId"/>\n';
		}
		xml = xml+'<activiti:in source="JG_LOCAL_PROCESS_ID" target="JG_SUB_MAIN_PROCESS_ID"/>\n';
		//update-end---author:scott------------date:20160308-----------------for:默认传递变量给子流程-------
	}
