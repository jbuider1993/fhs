...
public class EConfig implements InitializingBean, CommandLineRunner
{
    ...
    private void initConfigFromProperties(){
        ...
        InputStream in = EConfig.class.getClassLoader().getResourceAsStream("path.properties");
        // 使用properties对象加载输入流
        try {
            PATH.load(in);
            in = EConfig.class.getClassLoader().getResourceAsStream("other.properties");
            OTHER_CONFIG.load(in);
        } catch (IOException e) {
